...
exports (Uncaller, makeUncaller)

# Copyright 2003 Hewlett Packard, Inc. under the terms of the MIT X license
# found at http://www.opensource.org/licenses/mit-license.html ................

#@@ import "@@" =~ [=> makeRemoteCall :DeepFrozen]
...
object minimalUncaller as DeepFrozen implements Uncaller {
    method optUncall(obj) :NullOk[Portrayal] {
        if (Ref.isNear(obj)) {
            ...
            throw("@@not implemented: makeRemoteCall.optUncall(obj)")
        }
    }
    method _muteSMO() {}
}

def minimalUncallers :DeepFrozen := [minimalUncaller]
...
object makeUncaller as DeepFrozen {
    ...
    #  *"
    method onlySelfless(baseUncallers) :Uncaller {

        object onlySelflessUncaller {
            to optUncall(obj) :NullOk[Portrayal] {
                if (Ref.isSelfless(obj)) {
                    for baseUncaller in (baseUncallers) {
                        if (baseUncaller.optUncall(obj) =~ result :NotNull) {
                            return result
                        }
                    }
                }
